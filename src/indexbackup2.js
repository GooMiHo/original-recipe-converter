  ...
  while ( remainingCups > 0.002 ) {
    for(let i = 0; i < cupsToTblOrTsp.length; i++) {
      if( remainingCups >= cupsToTblOrTsp[i].cup && remainingCups >= 0.003) {
        ...
        if(cupsToTblOrTsp[i].cup === cupsToTblOrTsp[0].cup || cupsToTblOrTsp[i].cup === cupsToTblOrTsp[1].cup && smallerRemainder > 0.002) {
          if(smallerRemainder < 0.03 && count > 1) {
          ...
          else if(smallerRemainder >= 0.003 && count > 2) {
...
function addNewItem(wordToFind, amountToConvert) {
    ...
    const recipeLis = itemsAndAmounts.map((item, i) => {
        ...
        return <li key={keyId} id={keyId}>{item} {myButton}</li>;
    });
...
document.getElementById("add-list-button").addEventListener("click", function() {
    ...
    ReactDOM.render(<ul>{recipeLis2}</ul>, document.getElementById('root2'));
});
